...
(function() {
    ...
    function getLentilleContextData () {
        ...
        try {
            ...
            console.error(
                'Better Luogu User Practice: fail to parse lentille context',
                err
            )
    ...
    async function fetchPracticeDataNoCache (uid) {
        if (!REGEXP_FULL_USER_ID.test(uid))
            return null
        ...
        try {
            ...
            console.error(
                'Better Luogu User Practice: fail to fetch practice data',
                err
            )
    ...
    async function fetchPracticeData (uid) {
        ...
        if (uid !== uid_in_fetch) {
            ...
            practice_data_in_fetch =
                fetchPracticeDataNoCache(uid)
        } else if (
            await practice_data_in_fetch === null
        ) {
    ...
    async function updateProblems () {
        ...
        for (let passed of data.passed) {
            problems[passed.pid] = {
                dif: passed.difficulty,
                rendered: false
            }
        }

        for (let tryed of data.submitted) {
            problems[tryed.pid] = {
                dif: tryed.difficulty,
                rendered: false
            }
        }
    ...
    function renderColor () {
        if (
            !REGEXP_FIND_URL_PRACTICE.test(
                window.location.href
            )
        ) {
            if (partRendered) {
                for (let pid in problems) {
                    getEditableProblemRecord(
                        pid
                    ).rendered = false
                }
    ...
    function renderChart () {
        ...
        for (let elDiv of elDivList) {
            ...
            if (
                elDiv
                    .__betterLuoguUserPractice_width ===
                width
            )
                continue
            ...
            if (!elChart) {
                ...
                elChart.classList.add(
                    '__blup_chart'
                )

                elChart.style.backgroundColor =
                    elCaption
                        ?.style
                        ?.backgroundColor

                elChart.style.position =
                    'absolute'
            ...
            let halfHeight =
                elChart.clientHeight / 2

            elChart.style.width =
                `${width}px`

            elChart.style.borderTopLeftRadius =
                `${halfHeight}px`

            elChart.style.borderBottomLeftRadius =
                `${halfHeight}px`
        }
    }
    ...
    window.__betterLuoguUserPractice_sortByDifficulty =
        true

    function sortProblemsCompare (
        elA,
        elB
    ) {
        let aPid = elA.textContent
        let bPid = elB.textContent
        ...
        if (
            window
                .__betterLuoguUserPractice_sortByDifficulty &&
            aRecord.dif != bRecord.dif
        ) {
            if (
                aRecord.dif <
                bRecord.dif
    ...
    function sortProblems () {
        ...
        for (
            ...
            if (
                elDiv
                    .__betterLuoguUserPractice_sortedCode ===
                sortedCode
            )
                continue
    ...
    async function initialMain () {
        ...
        while (true) {
            try {
                ...
                console.error(
                    'Better Luogu User Practice: error in main loop',
                    err
                )
